...
function SystemMap({ state, compact, displayConfidence, mode, output }: {
  ...
  const decColor = state.decision === "PUSH" ? "#1FBE7A" : state.decision === "TEST" ? "#2C6BFF" : "#F5B73D";
...
function LiveFeed({ feed, decision }: { feed: FeedEntry[]; decision: Decision }) {
  const decColor = decision === "PUSH" ? "text-push" : decision === "TEST" ? "text-test" : "text-hold";
...
function FragmentVelocity({ active, decision }: { active: boolean; decision: Decision }) {
  const color = decision === "PUSH" ? "#1FBE7A" : decision === "TEST" ? "#2C6BFF" : "#F5B73D";
